...
if obj_editor_gui.mode = 4 {
	if placed = 1 {
		if !instance_exists(obj_editor_button_parent) {
			if mouse_x >= self.x && mouse_x < self.x+width*20 && (obj_editor_gui.mode = 4 && (mouse_y >= self.y && mouse_y < self.y+image_yscale*20)) {
				for (i = 0; i < instance_number(obj_trigger_parent); i += 1) {
					if (obj_editor_gui.mode = 4 && (mouse_x >= instance_find(obj_trigger_parent,i).x && mouse_x < instance_find(obj_trigger_parent,i).x+instance_find(obj_trigger_parent,i).width*20 && mouse_y >= instance_find(obj_trigger_parent,i).y && mouse_y < instance_find(obj_trigger_parent,i).y+instance_find(obj_trigger_parent,i).image_yscale*20) || obj_editor_gui.mode = 1 && (mouse_x >= instance_find(obj_trigger_parent,i).x && mouse_x < instance_find(obj_trigger_parent,i).x+instance_find(obj_trigger_parent,i).width*20 && mouse_y >= instance_find(obj_trigger_parent,i).y+(instance_find(obj_trigger_parent,i).zfloor-instance_find(obj_trigger_parent,i).zcieling)*20 && mouse_y < instance_find(obj_trigger_parent,i).y+instance_find(obj_trigger_parent,i).image_yscale*20)) {
	...
	// Depth
	depth = -(y + (height+zfloor)*20) - (zfloor) - 2;
}
