		...
		if instance_exists(trg) {
			...
			if trg.select {
				...
				if vertexToInd != -1 {
					...
					if instance_exists(obj_region_button_edge) {
						if !collision_rectangle(relativeMouseX-1,relativeMouseY-1 + (self.zfloor * 20),relativeMouseX+1,relativeMouseY+1 + (self.zfloor * 20),obj_trigger_vertex_pan,false,false) { // Prevent interference with deleting vertices
